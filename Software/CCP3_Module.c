...
static int8_t CCP3_Module_Data_Adjustment ( bool left_adj )
{
    // Check for the data adjustment
    if( left_adj )
    {
        CCP3_CCP3CON_LEFT_ADJUST_DATA();
    }
    else
    {
        CCP3_CCP3CON_RIGHT_ADJUST_DATA();
    }
...
static int8_t CCP3_Module_Set_Capt_Input ( uint8_t input )
{
    // Check to see if the Data are in the correct range
    if( input & ~CCP3_CCP3CAP_CTS_MASK )
    {
        // Data Input scope is out of range, return error
        return -1;
    }
    // Reset the Capture Input Register
    CCP3CAP &= ~CCP3_CCP3CAP_CTS_MASK;
...
static int8_t CCP3_Module_Set_Timer ( uint8_t timer_sel )
{
    // Check to see if the Data are in the correct range
    if( ( timer_sel & ~CCP3_CCPTMRS0_C3TSEL_MASK ) || ( 0 == timer_sel ) )
    {
        // Data Input scope is out of range, return error
        return -1;        
    }
    // Reset the Timer Input
    CCPTMRS0 &= ~CCP3_CCPTMRS0_C3TSEL_MASK;
                             ...
                             bool left_adj, bool int_en, bool int_hi_pri, bool en )
{
    ...
    temp_status += CCP3_Module_Set_Mode( mode );
    // Configure the Timer that should operate with the Comparator
    temp_status += CCP3_Module_Set_Timer( timer_sel );
    // Configure the capture input to be used
    temp_status += CCP3_Module_Set_Capt_Input( capt_in );
    ...
    if ( int_en )
    {
        ...
        CCP3_CLEAR_INTERRUPT_FLAG();
        // Enable the Interrupt
        CCP3_ENABLE_INTERRUPT();
    }
    else
    {
        // Disable the Interrupt
        CCP3_DISABLE_INTERRUPT();
    }
    // Configure the Interrupt priority
    if ( int_hi_pri )
    {
        // Interrupt set to high priority
        CCP3_INTERRUPT_SET_HIGH_PRIORITY();
    }
    else
    {
        // Interrupt set to low priority
        CCP3_INTERRUPT_SET_LOW_PRIORITY();
    }
...
bool CCP3_Module_Get_OUT_State ( void )
{
    ...
    if( CCP3CON & CCP3_CCP3CON_OUT_MASK )
    {
        // OUT is Set
        return true;
    }
...
{
    // Check to see the OUT bit state
    if( CCP3CON & CCP3_CCP3CON_FMT_MASK )
    {
...
uint8_t CCP3_Module_Get_Mode ( void )
{
    ...
    return ( CCP3CON & CCP3_CCP3CON_MODE_MASK );
}
...
{
    // Temporary Variables
    uint16_t temp_HIGH = CCPR3H;
    uint16_t temp_LOW  = CCPR3L;
    ...
    if ( 0x0C <= ( CCP3CON & CCP3_CCP3CON_MODE_MASK ) )
    {
...
void __interrupt( irq( IRQ_CCP3 ) ) CCP3_Isr ( void )
{
