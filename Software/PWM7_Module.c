...
// Sets the Timer Source for the PWM 7 Module
static int8_t PWM_7_Set_Timer_Source ( uint8_t timer ) {
    // Check the input if it is in range 0x01 - 0x03
    if ( !( CCPTMRS1_P7TSEL_MASK & timer ) ) {
        // Exit with Error
        return -1;
    }
    // Set the register without changing other bits
    CCPTMRS1 = ( CCPTMRS1 & ~CCPTMRS1_P7TSEL_MASK ) | timer;
...
// Sets the Enable / Disable of the PWM 7 Module
...
// Get the Enable / Disable Status of PWM 7 Module
bool PWM_7_Get_Enable ( void ) {
    // Returns the enable bit of the configuration register
    return PWM7CONbits.EN;
}

// Sets the Polarity bit of the PWM 7 Module
...
// Inverts the polarity of the output of PWM 7 Module
...
// Get the Current Polarity of the output of PWM 7 Module
bool PWM_7_Get_Polarity ( void ) {
    // Returns the polarity bit of the configuration register
    return PWM7CONbits.POL;
}

// Returns the Timer Source of PWM 7 Module
uint8_t PWM_7_Get_Timer_Source ( void ) {
    // Return the Timer settings of PWM 7 Module
    return ( CCPTMRS1 & CCPTMRS1_P7TSEL_MASK );
}
...
int8_t PWM_7_Set_Output_No_Cap ( uint16_t pwm_val ) {
    ...
    PWM7DCH = ( uint8_t ) ( ( pwm_val >> 2 ) & 0xFF );
    PWM7DCL = ( uint8_t ) ( ( pwm_val & 0x03 ) << 0x06 );
...
int8_t PWM_7_Set_Output_Capped ( uint16_t pwm_val ) {
    ...
    switch ( PWM_7_Get_Timer_Source ( ) ) {
        // Case TMR2 is used, cap based on TMR2PR
        case CCPTMRS1_P7TSEL_TMR2 : {
            // Temporary variable
...
int8_t PWM_7_Initialize ( uint8_t timer, bool out_inv, bool start ) {
    ...
    PWM_7_Set_Polarity ( out_inv );
    // If start is forced the start the PWM output
    PWM_7_Set_Enable ( start );
    // Return to the point of call with success
    return 1;
}