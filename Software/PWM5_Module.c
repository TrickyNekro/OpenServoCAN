...
static int8_t PWM_5_Set_Timer_Source ( uint8_t timer ) {
    // Check the input if it is in range 0x01 - 0x03
    if ( !( CCPTMRS1_P5TSEL_MASK & timer ) ) {
        // Exit with Error
        return -1;
    }
    // Set the register without changing other bits
    CCPTMRS1 = ( CCPTMRS1 & ~CCPTMRS1_P5TSEL_MASK ) | timer;
...
// Sets the Enable / Disable of the PWM 5 Module
...
// Get the Enable / Disable Status of PWM 5 Module
bool PWM_5_Get_Enable ( void ) {
    // Returns the enable bit of the configuration register
    return PWM5CONbits.EN;
}

// Sets the Polarity bit of the PWM 5 Module
...
// Inverts the polarity of the output of PWM 5 Module
...
// Get the Current Polarity of the output of PWM 5 Module
bool PWM_5_Get_Polarity ( void ) {
    // Returns the polarity bit of the configuration register
    return PWM5CONbits.POL;
}

// Returns the Timer Source of PWM 5 Module
uint8_t PWM_5_Get_Timer_Source ( void ) {
    // Return the Timer settings of PWM 5 Module
    return ( CCPTMRS1 & CCPTMRS1_P5TSEL_MASK );
}
...
int8_t PWM_5_Set_Output_No_Cap ( uint16_t pwm_val ) {
    ...
    PWM5DCH = ( uint8_t ) ( ( pwm_val >> 2 ) & 0xFF );
    PWM5DCL = ( uint8_t ) ( ( pwm_val & 0x03 ) << 0x06 );
...
int8_t PWM_5_Set_Output_Capped ( uint16_t pwm_val ) {
    ...
    switch ( PWM_5_Get_Timer_Source ( ) ) {
        // Case TMR2 is used, cap based on TMR2PR
        case CCPTMRS1_P5TSEL_TMR2 : {
            // Temporary variable
...
int8_t PWM_5_Initialize ( uint8_t timer, bool out_inv, bool start ) {
    ...
    PWM_5_Set_Polarity ( out_inv );
    // If start is forced the start the PWM output
    PWM_5_Set_Enable ( start );
    // Return to the point of call with success
    return 1;
}