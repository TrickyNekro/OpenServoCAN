...
static int8_t CCP2_Module_Data_Adjustment ( bool left_adj )
{
    // Check for the data adjustment
    if( left_adj )
    {
        CCP2_CCP2CON_LEFT_ADJUST_DATA();
    }
    else
    {
        CCP2_CCP2CON_RIGHT_ADJUST_DATA();
    }
...
static int8_t CCP2_Module_Set_Capt_Input ( uint8_t input )
{
    // Check to see if the Data are in the correct range
    if( input & ~CCP2_CCP2CAP_CTS_MASK )
    {
        // Data Input scope is out of range, return error
        return -1;
    }
    // Reset the Capture Input Register
    CCP2CAP &= ~CCP2_CCP2CAP_CTS_MASK;
...
static int8_t CCP2_Module_Set_Timer ( uint8_t timer_sel )
{
    // Check to see if the Data are in the correct range
    if( ( timer_sel & ~CCP2_CCPTMRS0_C2TSEL_MASK ) || ( 0 == timer_sel ) )
    {
        // Data Input scope is out of range, return error
        return -1;        
    }
    // Reset the Timer Input
    CCPTMRS0 &= ~CCP2_CCPTMRS0_C2TSEL_MASK;
                             ...
                             bool left_adj, bool int_en, bool int_hi_pri, bool en )
{
    ...
    temp_status += CCP2_Module_Set_Mode( mode );
    // Configure the Timer that should operate with the Comparator
    temp_status += CCP2_Module_Set_Timer( timer_sel );
    // Configure the capture input to be used
    temp_status += CCP2_Module_Set_Capt_Input( capt_in );
...
bool CCP2_Module_Get_OUT_State ( void )
{
    ...
    if( CCP2CON & CCP2_CCP2CON_OUT_MASK )
    {
        // OUT is Set
        return true;
    }
...
{
    // Check to see the OUT bit state
    if( CCP2CON & CCP2_CCP2CON_FMT_MASK )
    {
...
uint8_t CCP2_Module_Get_Mode ( void )
{
    ...
    return ( CCP2CON & CCP2_CCP2CON_MODE_MASK );
}
...
{
    // Temporary Variables
    uint16_t temp_HIGH = CCPR2H;
    uint16_t temp_LOW  = CCPR2L;
    ...
    if ( 0x0C <= ( CCP2CON & CCP2_CCP2CON_MODE_MASK ) )
    {
...
void __interrupt( irq( IRQ_CCP2 ) ) CCP2_Isr ( void )
{
