...
// Sets the Timer Source for the PWM 6 Module
static int8_t PWM_6_Set_Timer_Source ( uint8_t timer ) {
    // Check the input if it is in range 0x01 - 0x03
    if ( !( CCPTMRS1_P6TSEL_MASK & timer ) ) {
        // Exit with Error
        return -1;
    }
    // Set the register without changing other bits
    CCPTMRS1 = ( CCPTMRS1 & ~CCPTMRS1_P6TSEL_MASK ) | timer;
...
// Sets the Enable / Disable of the PWM 6 Module
...
// Get the Enable / Disable Status of PWM 6 Module
bool PWM_6_Get_Enable ( void ) {
    // Returns the enable bit of the configuration register
    return PWM6CONbits.EN;
}

// Sets the Polarity bit of the PWM 6 Module
...
// Inverts the polarity of the output of PWM 6 Module
...
// Get the Current Polarity of the output of PWM 6 Module
bool PWM_6_Get_Polarity ( void ) {
    // Returns the polarity bit of the configuration register
    return PWM6CONbits.POL;
}

// Returns the Timer Source of PWM 6 Module
uint8_t PWM_6_Get_Timer_Source ( void ) {
    // Return the Timer settings of PWM 6 Module
    return ( CCPTMRS1 & CCPTMRS1_P6TSEL_MASK );
}
...
int8_t PWM_6_Set_Output_No_Cap ( uint16_t pwm_val ) {
    ...
    PWM6DCH = ( uint8_t ) ( ( pwm_val >> 2 ) & 0xFF );
    PWM6DCL = ( uint8_t ) ( ( pwm_val & 0x03 ) << 0x06 );
...
int8_t PWM_6_Set_Output_Capped ( uint16_t pwm_val ) {
    ...
    switch ( PWM_6_Get_Timer_Source ( ) ) {
        // Case TMR2 is used, cap based on TMR2PR
        case CCPTMRS1_P6TSEL_TMR2 : {
            // Temporary variable
...
int8_t PWM_6_Initialize ( uint8_t timer, bool out_inv, bool start ) {
    ...
    PWM_6_Set_Polarity ( out_inv );
    // If start is forced the start the PWM output
    PWM_6_Set_Enable ( start );
    // Return to the point of call with success
    return 1;
}