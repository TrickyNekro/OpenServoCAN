                            ...
                            uint8_t start_sync, uint8_t mode, bool up_live )
{
    ...
    if((T2HLT_PSYNC_ON != pres_sync) || (T2HLT_PSYNC_OFF != pres_sync))
    {
        // Return a fault code
        return -1;
    }
    // Illegal values of T2HLT.CPOL
    if((T2HLT_CKPOL_RISING != pres_edge) || (T2HLT_CKPOL_FALLING != pres_edge))
    {
        // Return a fault code
        return -1;
    }
    // Illegal values of T2HLT.CKSYNC
    if((T2HLT_CKSYNC_ON != start_sync) || (T2HLT_CKSYNC_OFF != start_sync))
    {
        // Return a fault code
        return -1;
    }
...
static int8_t Timer_2_T2RST_Set ( uint8_t res_source, bool up_live )
{
    ...
    if((0x01 == res_source) || (0x0C == res_source) || (0x0D == res_source) ||
            0x19 <= res_source)
    {
        // Return a fault code
        return -1;
